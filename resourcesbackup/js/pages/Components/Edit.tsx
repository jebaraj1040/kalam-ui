...
import { Field, type BreadcrumbItem } from '@/types';
...
export default function EditComponent() {
    const { component: initialComponent, breadcrumbs, component_count, tags, all_tags } = usePage<PageProps>().props;

    // Convert schema from backend into internal format
    const transformSchemas = useCallback((schemas: any[]): Schema[] => {
        return schemas.map((s) => ({
            ...
            fields: (s.fields || []).map((f: any) => {
                ...
                // Handle Image or Link nested fields
                if (f.type === 'Image' || f.type === 'Link') {
                    if (!Array.isArray(f.fields) || f.fields.length === 0) {
                        ...
                        // Reattach and ensure ids for existing subfields
                        field.fields = f.fields.map((sf: any) => ({
                            id: sf.id || uuidv4(),
                            type: sf.type,
                            key: sf.key,
                            label: sf.label,
                ...
                } else if (Array.isArray(f.fields) && f.fields.length > 0) {
                    // For other types (rare case with nested fields)
                    field.fields = f.fields.map((sf: any) => ({
                        id: sf.id || uuidv4(),
                        type: sf.type,
                        key: sf.key,
            ...
            children: s.children?.length ? transformSchemas(s.children) : [],
        }));
    }, []);

    // Transform Mongo DB _id to plain id
    const transformComponent = useCallback(
        (data: PageProps['component']): Component => {
            return {
                id: data.id,
                component_name: data.component_name,
                status: data.status,
                description: data.description,
                schema: transformSchemas(data.schema || []),
            };
        },
        [transformSchemas],
    );
    ...
    const [schemeShow, setSchemaShow] = useState(true);
    const [availableTags, setAvailableTags] = useState<Tag[]>(all_tags);
    const [createTags, setCreateTags] = useState<Tag[]>(tags);
    ...
    useEffect(() => {
        ...
        fetch(`/components/active?search=${encodeURIComponent(searchTerm)}&cid=${initialComponent.id}`, { signal: controller.signal })
            .then((res) => res.json())
    ...
    useEffect(() => {
        const c = transformComponent(initialComponent);
        setComponentId(c.id);
        setComponentName(c.component_name);
        setStatus(c.status);
        setDescription(c.description);
        setSchema(c.schema);
    }, [initialComponent, transformComponent]);
    ...
    const addField = useCallback(
        (schemaId: string) => {
            const newField: Field = { id: uuidv4(), type: 'Text', key: '', label: '' };
            setSchema((prev) => updateSchemaArray(prev, schemaId, (s) => ({ ...s, fields: [...s.fields, newField] })));
        },
        [updateSchemaArray],
    );
    ...
    const handleSubmit = useCallback(() => {
        ...
        router.put(
            ...
            {
                onSuccess: () => console.log('Component updated'),
                onError: (err) => {
                    console.error(err);
    ...
    }, [componentId, componentName, description, createTags, existingTagIds, schema, status, validateSchemas]);
            ...
            <div>
                <div className="flex items-center justify-between px-6 py-5">
                    <p className="font-tree-regular text-base text-font-primary">
                        Components <span className="font-tree-semibold">/ Edit Component</span>
                    </p>
